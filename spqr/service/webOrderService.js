            ...
            orderMap.forEach((subOrders, tableNumber) => {
                subOrders.forEach((subOrder) => {
                    ...
                    if (orderStatus === "Accepted") {
                        const existingAcceptedOrder = acceptedOrders.find((order) => order.table_number === tableNumber);
                        if (existingAcceptedOrder) {
                            existingAcceptedOrder.sub_orders.push(categorizedSubOrder.sub_order);
                        } else {
                            acceptedOrders.push({
                                table_number: tableNumber,
                        ...
                        const existingPendingOrder = pendingOrders.find((order) => order.table_number === tableNumber);
                        if (existingPendingOrder) {
                            existingPendingOrder.sub_orders.push(categorizedSubOrder.sub_order);
                        } else {
                            pendingOrders.push({
                                table_number: tableNumber,
                                sub_orders: [categorizedSubOrder.sub_order],
